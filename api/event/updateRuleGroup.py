...
# set logger
cLog = cLogger.cLogger("/api/event/updateRuleGroup")
...
def send_api(param):
  path = "/cw_fea/real/cw/api/rule/group/ruleGrp/update"
  ...
  body["suspendRuleItems"] = param['suspendRuleItems']
  ...
  if rsltData['status'] == 200:
    ...
    print("이벤트 룰 수정 실패 [ %s ]" % rsltData['data']['msg'])
# end def
